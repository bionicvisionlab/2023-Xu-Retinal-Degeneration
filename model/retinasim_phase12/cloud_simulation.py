...
def cloud_to_retina_output(sp, light_g_max, pre_computed_weights_folder, save_folder, res=32, select_GPU=None, genn_directory=None):
    ...
    for batch_number in range(29, num_batches):
        ...
        _, _, _, _, _, _, _, _, _, spikes_gl_on, spikes_gl_off = stim_to_retina_output(num_timesteps_per_batch*cloud_dt, sp, light_g_max, pre_computed_weights_folder, lstim=stim, dt=0.1, select_GPU=select_GPU, genn_directory=genn_directory)
...
if __name__ == "__main__":
    ...
    sp_txt = "parameter/cottaris_original_2hz.txt"
    sp = SimulationParam(sp_txt)
    sp.xy_coord_folder = "dendritic-tree-coordinate-{}".format(cone_population)
    sp.z_coord_folder = "z-coordinate-{}".format(cone_population)
    
    save_folder = "data/data-cloud-stim-{}-{}-2hz".format(light_g_max, cone_population)
    ...
    cloud_to_retina_output(sp, light_g_max, "weights", save_folder, res=res, select_GPU=select_GPU, genn_directory=genn_directory)