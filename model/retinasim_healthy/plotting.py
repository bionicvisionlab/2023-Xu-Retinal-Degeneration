...
#     interp_x_y = np.load(path_to_interp_x_y)
#     heatmap = axs[0].imshow(np.transpose(interp_x_y[:,:,50]), vmin=v_min, vmax=v_max, origin='lower', cmap=get_cmap(), aspect='auto', extent=[0,55000,0,100])
#     axs[0].set_xticks(np.arange(0,16000,1000))
#     axs[0].set_xticklabels(np.arange(0,1600,100))
#     axs[0].set_yticks([0,20,40,60,80,100])
#     axs[0].set_yticklabels([0,60,120,180,240,300])
#     axs[0].set_ylabel('Space in y dimension (μm)', fontsize=10)
#     for xc in vert_l:
...
#     for i in cell_index:
#         st_example = st['t'][i]/ms
# #         st_example = (st_example[np.logical_and(st_example>500, st_example<2000)] - 500)*10
#         st_example = (st_example[np.logical_and(st_example>500, st_example<6000)] - 500)*10
#         data.append(st_example)
#     data_c = np.concatenate([data], axis=0) # could break if the number of spikes are unequal
#     axs[1].eventplot(data_c, colors='black', lineoffsets=1, linelengths=1)
#     axs[1].yaxis.set_visible(False)
