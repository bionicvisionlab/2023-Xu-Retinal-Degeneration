...
from model.retinasim_phase3.retina_network import retina_simulation

# sp can be either the path to a txt file containing the simulation parameters or a SimulationParam object

def estim_to_retina_output(time_in_ms, sp, estim, pre_computed_weights_folder, light_intensity=0.5, delta_ve_folder="delta_ve_workspace", dt=0.01, debug=False, gpu=True, select_GPU=None, genn_directory=None):
    ...
    data = retina_simulation(time_in_ms, sp, lstim, pre_computed_weights_folder=pre_computed_weights_folder, delta_ve_folder=delta_ve_folder, delta_ve_dt=dt, simulation_timestep=dt, debug=debug, gpu=gpu, select_GPU=select_GPU, genn_directory=genn_directory)
