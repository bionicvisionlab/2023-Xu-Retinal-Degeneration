...
#     NUM_CR = np.load("{}/CR.npy".format(sp.xy_coord_folder)).shape[1]
    ...
    lstim = TimedArray(np.ones((time_in_ms, 1)) * 0.5, dt=1*ms)

    data = retina_simulation(time_in_ms, sp, injected_current_on, injected_current_off, lstim, pre_computed_weights_folder=pre_computed_weights_folder, delta_ve_folder=None, delta_ve_dt=dt, simulation_timestep=dt, debug=debug, gpu=gpu, select_GPU=select_GPU, genn_directory=genn_directory)
