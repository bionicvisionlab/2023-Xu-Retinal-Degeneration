...
def gap_junction_src_tgt(path_to_coord_1, path_to_coord_2, radius=30):
    coord_1 = np.load(path_to_coord_1)
    coord_2 = np.load(path_to_coord_2)
    dist_mat = distance_matrix(np.transpose(coord_1), np.transpose(coord_2))
    if path_to_coord_1 == path_to_coord_2:
        con = np.logical_and(dist_mat < radius, dist_mat != 0)
    else:
        con = dist_mat < radius
    src, tgt = con.nonzero()
    return src, tgt 
...
def retina_simulation(time_in_ms, sp, injected_current_on, injected_current_off, lstim, light_g_max=0.9, pre_computed_weights_folder=None, delta_ve_folder=None, delta_ve_dt=0.01, simulation_timestep=None, debug=False, gpu=True, select_GPU=None, genn_directory=None):
