...
def get_distances(path_to_coord_1, path_to_coord_2, sigma_2):
    ...
    D = dist_mat.flatten()
    W = np.tile(np.sum(np.exp(-dist_mat/sigma_2), axis=0), num_1)
    return D, W

# initial membrane voltages are the resting voltage when light intensity is 0.5
def retina_simulation(time_in_ms, sp, injected_current_on, injected_current_off, lstim, delta_ve_folder=None, delta_ve_dt=0.01, simulation_timestep=None, debug=False, gpu=True, select_GPU=None, genn_directory=None):
    ...
    if sp.cone_exists:
        ...
        V_delayed_bp_off : 1
        V_value = V/mV : 1
        syn_delay_hrz : second
        syn_delay_bp_on : second
        syn_delay_bp_off : second
        '''
        ...
        cr.syn_delay_hrz = 7 * ms
        cr.V_delayed_hrz = 0
        cr.syn_delay_bp_on = 5 * ms
        cr.V_delayed_bp_on = 0
        cr.syn_delay_bp_off = 13 * ms
        cr.V_delayed_bp_off = 0
    ...
    bp_off.G_syn = 0
    # synaptic delay of amacrine connections
    bp_off.syn_delay_am = 11 * ms
    bp_off.V_delayed_am = 0
    # synaptic delay of ganglion connections
    bp_off.syn_delay_gl = 13 * ms
    ...
    am_nf_on.G_syn = 0
    am_nf_on.syn_delay = 12 * ms
    am_nf_on.V_delayed = 0

    # ganglion on
    gl_on_eqs = '''
    dV/dt = (1/C_m) * (I_e - I_inj + I_syn - I_m) : volt
    # injected current for FI curve
    I_inj = ic * (uA/cm**2) * sa : amp 
    ic : 1
    ...
    # get delayed voltage
    if sp.cone_exists:
        cr.run_regularly("V_delayed_hrz = get_delayed_cr(i, timestep(t-syn_delay_hrz, dt))")
        cr.run_regularly("V_delayed_bp_on = get_delayed_cr(i, timestep(t-syn_delay_bp_on, dt))")
        cr.run_regularly("V_delayed_bp_off = get_delayed_cr(i, timestep(t-syn_delay_bp_off, dt))")
    ...
    if sp.cone_exists:
        cr.run_regularly("G_syn_l = (0 + (0.9-0)*(1-lstim(t,i))) * nsiemens")
        
    # injected_current
    gl_on.run_regularly("ic = injected_current_on(t, i)")
    gl_off.run_regularly("ic = injected_current_off(t, i)")
    ...
    if debug:
        ...
        gl_on_mon = StateMonitor(gl_on, ["V", "I_syn", "I_syn_a", "I_syn_b", "G_syn_a", "G_syn_b", "I_e", "I_inj"], record=True)
        gl_off_mon = StateMonitor(gl_off, ["V", "I_syn", "G_syn_aw", "G_syn_an", "G_syn_b", "I_e", "m", "n", "h", "I_inj"], record=True)
    else:
        if sp.cone_exists:
    ...
    if sp.cone_exists:
        cr_hrz = Synapses(cr, hrz, model=synapse_params+increasing_synapse_eqs.replace("V_delayed_pre", "V_delayed_hrz_pre"), method='exponential_euler')
        cr_hrz.connect()
        ...
        cr_bp_on = Synapses(cr, bp_on, model=synapse_params+decreasing_synapse_eqs.replace("V_delayed_pre", "V_delayed_bp_on_pre"), method='exponential_euler')
        cr_bp_on.connect()
        ...
        cr_bp_off = Synapses(cr, bp_off, model=synapse_params+increasing_synapse_eqs.replace("V_delayed_pre", "V_delayed_bp_off_pre"), method='exponential_euler')
        cr_bp_off.connect()
